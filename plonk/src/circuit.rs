...
impl Circuit<> {
    ...
    pub fn compile_circuit(&mut self) -> Result<CompiledCircuit, &str> {
        ...
        let interpolated_assignment = assignment.into_iter().map(|(k, v)| (k, Evaluations::from_vec_and_domain(v, domain).interpolate())).collect::<HashMap<_, _>>();

        let roots = domain.elements().collect::<Vec<_>>();
        let w = roots.get(0).unwrap();
        let tmp = interpolated_assignment.get(Self::VEC_A).unwrap().evaluate(w) * interpolated_assignment.get(Self::VEC_B).unwrap().evaluate(w) * interpolated_assignment.get(Self::VEC_QM).unwrap().evaluate(w)
            + interpolated_assignment.get(Self::VEC_A).unwrap().evaluate(w) * interpolated_assignment.get(Self::VEC_QL).unwrap().evaluate(w)
            + interpolated_assignment.get(Self::VEC_B).unwrap().evaluate(w) * interpolated_assignment.get(Self::VEC_QR).unwrap().evaluate(w) + interpolated_assignment.get(Self::VEC_QO).unwrap().evaluate(w) * interpolated_assignment.get(Self::VEC_C).unwrap().evaluate(w) + interpolated_assignment.get(Self::VEC_QC).unwrap().evaluate(w) + interpolated_assignment.get(Self::VEC_PI).unwrap().evaluate(w);
        if tmp != Fr::from(0) {
        ...
        Ok(CompiledCircuit::new(gate_constraints, copy_constraints, srs, domain, len))
    }
}
